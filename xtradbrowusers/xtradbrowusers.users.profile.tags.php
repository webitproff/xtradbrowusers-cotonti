...
/**
 * Filename: xtradbrowusers.users.profile.tags.php – Вывод полей в форме редактирования профиля пользователем
 *
 * Purpose:   Подключается к хуку `users.profile.tags`, который вызывается в файле
 *            /modules/users/inc/users.profile.php перед парсингом шаблона.
 *            Отображает все экстраполя плагина в форме редактирования профиля пользователем.
 *            Назначает теги вида:
 *            {USERS_PROFILE_XTRA_ИМЯПОЛЯ}, {USERS_PROFILE_XTRA_ИМЯПОЛЯ_TITLE},
 *            а также общий тег {USERS_PROFILE_XTRA_EXTRAFLD} и
 *            {USERS_PROFILE_XTRA_EXTRAFLD_TITLE}.
 ...
 *            с тегами {USERS_PROFILE_XTRA_ИМЯПОЛЯ_ЯЗЫК} и {USERS_PROFILE_XTRA_ИМЯПОЛЯ_ЯЗЫК_TITLE}.
 ...
 *
 * Path:     plugins/xtradbrowusers/xtradbrowusers.users.profile.tags.php
 *
...
if (!empty($extrafields) && !empty($urr['user_id'])) {
    ...
    // === Исправление (v1.1.1): мультиязычные поля ввода теперь отображаются только для текстовых типов ===
    ...
    foreach ($extrafields as $exfld) {
        ...
        if ($i18nEnabled && !empty($activeLangs) && in_array($exfld['field_type'], $i18nAllowedTypes)) {
            foreach ($activeLangs as $lang) {
                ...
                $t->assign([
                    'USERS_PROFILE_XTRA_' . strtoupper($exfld['field_name']) . '_' . strtoupper($lang) => $i18nElement,
                    'USERS_PROFILE_XTRA_' . strtoupper($exfld['field_name']) . '_' . strtoupper($lang) . '_TITLE' => $i18nTitle,
                ]);
            }
        }
    }
